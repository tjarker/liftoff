...
/** Verilog emission for Chisel 6: firtool writes one SystemVerilog file per module and lists
...
private[chisel] object VerilogEmitter {
  ...
  def emit(name: String, gen: => chisel3.RawModule, targetDir: File): Seq[File] = {
    ...
    // Layers are not in filelist.f, but their code only takes effect if their files are compiled.
